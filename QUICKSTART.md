...
The profile page is available from `View → Profile` or the user icon in the title bar. Sign in with the central Auth0 account, choose an avatar/background, and publish. SafeLauncher suggests a readable public username and stores only the rotating Auth0 refresh token in the OS credential store. Other users can open the username from `Open Public Profile…`; they receive a read-only view. You can rename a published username later; previous usernames continue to open the same profile.
